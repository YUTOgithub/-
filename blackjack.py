...
def get_point(hand):
    ...
    if ace_flag and result <= 11: #Aが含まれていて、合計が11以下か？
        result += 10 #Ａを11と考え、resultに10を加える
    #print(result)
    return result
...
def main ():
    turn = 1
    player_money = 100
    deck = make_deck()#これで山札が足りなくならないので、ここに移動

    while player_money > 0 :

        #ターンのはじめにターン数と所持金の情報を表示
        print("-"*20) #区切り線を作る。"-"を20個表示
        print("ターン：",turn)
        ...
        print("-"*20)#手札の情報を表示
        print_player_hand(player_hand)
        print_dealer_hand(dealer_hand,False)     #本来はTrueですが、ここではFalseにしておく
        print("-"*20)
